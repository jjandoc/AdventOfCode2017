...
class Program {
  ...
  /**
   * Returns true if this program has either no children (so it's the rightmost
   * in our object), or all its children have been defined.
   */
  isReadyToSort() {
...
function getProgramsToSort(tree) {
  return Object.values(tree).filter((item) => item.isReadyToSort());
}

function constructTree(input) {
  const tree = input.split('\n').map(row => {
    const [itemDetails, itemChildren] = row.split(' -> ');
    const [itemName, itemWeight] = itemDetails.split(' ');
    return new Program(itemName, itemWeight.replace(/[()]/g, ''), itemChildren)
  }).reduce((obj, item) => {
    obj[item.name] = item;
    return obj;
  }, {});
  ...
  while (Object.keys(tree).length > 1) {
    const programsToSort = getProgramsToSort(tree);
    programsToSort.map((item) => {
      delete tree[item.name];
      return item;
    }).forEach((item) => {
      Object.values(tree).forEach((program) => {
        if (program.hasChild(item.name)) {
          tree[program.name].children[item.name] = item;
        }
      });
    });
  }
...
function getTotalChildWeights(program) {
  return Object.values(program.children).reduce((totals, child) => {
    totals[child.name] = child.weight +
        Object.values(getTotalChildWeights(child))
        .reduce((sum, n) => sum + n, 0);
    return totals
  }, {});
}

function getImbalancedStack(program) {
  let result = null;
  const weights = getTotalChildWeights(program);
  Object.entries(weights).forEach((entry) => {
    const [name, weight] = entry;
    let hasMatch = false;
    for (let key in weights) {
      if (weight === weights[key] && key !== name) {
        hasMatch = true;
      }
    }       
    if (!hasMatch) {
      result = program.children[name];
    }
  });
  return result;
}

function getNewWeight(program) {
  let state = getTotalChildWeights(program);
  let badApple = null;
  let stackToTest = getImbalancedStack(program);

  while (stackToTest) {
    badApple = stackToTest;
    stackToTest = getImbalancedStack(stackToTest);
    state = stackToTest ? getTotalChildWeights(badApple) : state;
  }

  const weightAdjustment = Object.values(state)
      .reduce((result, value, i, array) => {
        ...
        return result;
      }, []).reduce((difference, value, i) =>
          i === 0 ? value : difference - value, 0);

  return badApple.weight + weightAdjustment;
}

const myTree = constructTree(myInput);
console.log(myTree); // Part 1
console.log(getNewWeight(Object.values(myTree)[0])); // Part 2