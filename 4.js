const myInput = `oaoe rxeq vssdqtu xrk cjv yaoqp loo
...
function getTotalValidPhrases(input, filterFunction) {
  return input.split('\n').filter(filterFunction).length;
}

function filterByNonDuplicates(phrase) {
  let isValid = true;
  phrase.split(' ').forEach((word, index, array) => {
    if (array.lastIndexOf(word) !== index) {
      isValid = false
    }
  });
  return isValid;
...
function filterByNonAnagrams(phrase) {
  let isValid = true;
  phrase.split(' ').map((word) => word.split('').sort().join(''))
...
console.log(getTotalValidPhrases(myInput, filterByNonDuplicates)); // Part 1
console.log(getTotalValidPhrases(myInput, filterByNonAnagrams)); // Part 2
