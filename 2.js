const myInput = `493	458	321	120	49	432	433	92	54	452	41	461	388	409	263	58
...
function getMinMaxDifference(numArray) {
  return (Math.max(...numArray) - Math.min(...numArray));
}

function getEvenlyDivisibleValues(numArray) {
  const [dividend, divisor] = numArray.filter((testNumber, i, array) => {
    let passes = false;
    array.forEach((number) => {
      if ((testNumber % number === 0 || number % testNumber === 0) &&
          testNumber !== number) {
        passes = true;
      }
    });
...
function getCheckSum(input, filterFunction) {
  return input.split('\n').map((row) => row.split('\t'))
    .reduce((sum, row) =>
        filterFunction(row.map((number) => parseInt(number, 10))) + sum, 0);
}

getCheckSum(myInput, getMinMaxDifference); // First part.
getCheckSum(myInput, getEvenlyDivisibleValues); // Second part.